      ...
      <div class="left-container container">
                <div class="profile block"> <!-- PROFILE (MIDDLE-CONTAINER) -->
                    ...
                    <div class="profile-picture big-profile-picture clear">  
						<div class="image_ava">
						<img :src="`http://106.14.45.227:8080/${String(docInfomation[0].pic)}.png`"  width=150px  height=150px> 
						</div>
                    </div>
					<!-- <div class="profile-description">
                        <p class="scnd-font-color">ID:{{this.$store.state.doctorId}}</p>
                    </div> -->
					<div class="info">
                    <h1 class="user-name">{{ docInfomation[0].name}}</h1>
					<p>性别：{{ docInfomation[0].sex}}</P>
					<p>职称：{{ docInfomation[0].title}}</P>
					<p>所在医院：{{ docInfomation[0].hos}}</P>
					<p>所在科室：{{ docInfomation[0].department}}</P>
					<p>个人简介：{{ docInfomation[0].intro}}</P>
					</div>
					<div class="profile-description">
                        <p class="scnd-font-color">ID:{{this.$store.state.userId}}</p>
                    </div>
                </div>
      </div>
            ...
            <header class="block">
                <ul class="header-menu horizontal-list">
                  <router-link to="/homepage">
                    <li>
                        <a class="header-menu-tab" href="#2"><span class="icon fontawesome-user scnd-font-color"></span>预约列表</a>
                    </li>
                    </router-link>
                    <router-link to="/records">
                    <li>
                        <a class="header-menu-tab" href="#5"><span class="icon fontawesome-star-empty scnd-font-color"></span>问诊记录</a>
                    </li>
                     </router-link>                    
                </ul>
...
import calendardoc from '@/components/calendardoc.vue'
import CalendarDay from '@/components/calendarDay.vue'

import Table from '@/components/Table.vue'
import Uploader from '@/components/uploaderD.vue'
import UpdateUser from '@/components/updateUserD.vue'
import {getDocFun} from '@/service/userService.js'
...
export default {
  ...
  components: {
	...
	methods:{
		getDataList(){
	getDocFun({id:this.$store.state.userId}).then(res=>{
		let pic_1;
		let n=res.result.picture_url.length;
		if(n==32) pic_1=res.result.picture_url.substring(27,28);
		else pic_1=res.result.picture_url.substring(27,29);
		console.log("图片地址为："+pic_1);
		this.docInfomation.push({
			name:res.result.doctor_name,
			identification:res.result.identification,
			working_age:res.result.working_age,
			sex:res.result.sex,
			title:res.result.title,
			hos:res.result.hos_name,
			department:res.result.department,
			pic:pic_1,
			intro:res.result.introduction,
		});
		this.$store.commit("editInquiryDocName",this.docInfomation[0].name);
        console.log(this.docInfomation);
      }).catch(err=>{
         console.log(err);
      });
    },
