<template>
			<div class="donut-chart-block block"> <!-- DONUT CHART BLOCK (LEFT-CONTAINER) -->
                    <ul class="os-percentages horizontal-list">
                        <li>
                            <p class="ios os scnd-font-color">BMI</p>
                            <p class="os-percentage">{{this.PaiBMI.bmi}}<sup></sup></p>
                        </li>
                        <li>
                            <p class="mac os scnd-font-color">血压-低</p>
                            <p class="os-percentage">{{this.PaiBMI.diastolic_pressure}}<sup></sup></p>
                        </li>
                        <li>
                            <p class="linux os scnd-font-color">血压-高</p>
                            <p class="os-percentage">{{this.PaiBMI.systolic_pressure}}<sup></sup></p>
                        </li>
                        <li>
                            <p class="win os scnd-font-color">心率</p>
                            <p class="os-percentage">{{this.PaiBMI.heart_rate}}<sup></sup></p>
                        </li>
                    </ul>
					&nbsp;
                    <h3 class="titular" style="padding-left:118px">健康得分</h3>
                    <div class="donut-chart">
                        ...
                        <p class="center-date">{{this.evaluate3}}</p> 
                    </div>
...
<script>
import {getPatiBMIFun} from '@/service/userService'
  export default {
    name:"healthReport",
    components: {
  },
	...
	methods:{
		BMI_created(){
              getPatiBMIFun(
                ...
                console.log("BMIget成功")
                this.PaiBMI.height=res.result.height;
                this.PaiBMI.weight=res.result.weight;
                this.PaiBMI.bmi=res.result.bmi.substring(0,5);
                this.PaiBMI.heart_rate=res.result.heart_rate;
                this.PaiBMI.systolic_pressure=res.result.systolic_pressure;
				this.PaiBMI.diastolic_pressure=res.result.diastolic_pressure;
                this.generateAdvice(this.PaiBMI.height,
							this.PaiBMI.weight,
							this.PaiBMI.heart_rate,
							this.PaiBMI.diastolic_pressure,
							this.PaiBMI.systolic_pressure);
				}
				).catch(err=>{
                  console.log(err);
              });
          },
		  generateAdvice(height,weight,heartRate,lBloodPre,hBloodPre){
            let BMI=weight/Math.pow(height*0.01,2);
            let BMIstate;                              //计算BMI
            let BMInum;
            let advice1;
            if(BMI<18.5)
                {BMIstate="偏瘦";BMInum=-1;advice1="身体免疫力降低，有可能造成骨质疏松，可以适度增重哦！"}
            else if(BMI>=18.5&&BMI<24)                 //正常BMI
                {BMIstate="正常";BMInum=0;advice1="正常，要继续保持哦."}
            else if(BMI>=24&&BMI<=28)                  //超重                   
                {BMIstate="超重";BMInum=1;advice1="这可能会导致糖尿病，心脑血管疾病等的并发症，而且呼吸暂停的可能性也会增加，应该加强锻炼，饮食清淡哦！"}
            else {BMIstate="肥胖";BMInum=2;advice1="您要特别注意您的BMI指标已经亮起红灯了，肥胖会导致多种并发症如糖尿病，高血压，应加强锻炼，合理饮食。"}                           //肥胖

            let rateState;                             //心率状况
            let ratenum;
            let advice2;
            if(heartRate>=60&&heartRate<=100)          //正常心率
                {rateState="正常";ratenum=0;advice2="心率方面较为正常。"}
            else if(heartRate<60)                      //心率较低
                {rateState="较低";ratenum=-1;advice2="您的平均心率较低，平时应注意改善生活习惯，建议每天早睡早起、保持充足的睡眠时间与精力、戒烟戒酒、尽量避免熬夜、劳逸适度、保持规律的生活习惯，必要时就医治疗。"}
            else {rateState="较高"; ratenum=1;advice2="您的平均心率较高，应保持规律运动，如慢跑，登山等，不建议剧烈运动，饮食上可适度补充营养，多吃富含维生素和蛋白质的事物，必要时就医治疗。"}                         //心率较高

            let BloodState;                            //血压状况
            let Bloodnum;
            let advice3;
            if(lBloodPre>=60&&lBloodPre<=90&&hBloodPre>=90&&hBloodPre<=140)     
                {BloodState="正常"; Bloodnum=0;advice3="血压方面较为正常，应该继续保持合理饮食，少食油腻，辛辣的食物。"}                         //正常血压
            else if(lBloodPre<60&&hBloodPre<90)
               { BloodState="较低"; Bloodnum=-1;advice3="您的血压较低，日常生活中应注意是否有头晕，头痛，疲倦，面色发白等症状，如果有，应及时就医治疗，生活要有规律，防止过度疲劳。"}                                  //低血压
            else if(lBloodPre>90&&hBloodPre>140)
                {BloodState="较高";Bloodnum=1;advice3="您的血压较高，应及时就医，要特别注意饮食清淡，少食多盐，油腻，辛辣的食物，生活中应保持平和的心情。"}                          //高血压
            else {BloodState="正常";Bloodnum=0;advice3="血压方面较为正常，应该继续保持合理饮食，少食油腻，辛辣的食物。"}

			let evaluate2;
            let evaluate1=BMInum+ratenum+Bloodnum;
            switch(evaluate1)
            {
                case 0: 
					this.evaluate3="95分";
            ...
            let advice;
            advice="您好！根据提供的各项数据，您的健康指标为"+evaluate2+advice1+advice2+advice3;
        }
	},
